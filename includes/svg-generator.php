  ...
  class OG_SVG_Generator
  {
    ...
    public function generateSVG($post_id = null)
    {
      ...
      if ($colors === $this->color_schemes['gabriel']) {
        // Subtle geometric patterns for Gabriel's theme
        $svg .= '<circle cx="1050" cy="150" r="120" fill="rgba(59, 130, 246, 0.08)" opacity="0.6"/>';
        $svg .= '<circle cx="1100" cy="500" r="80" fill="rgba(6, 182, 212, 0.06)" opacity="0.8"/>';
        $svg .= '<circle cx="100" cy="100" r="60" fill="rgba(59, 130, 246, 0.05)" opacity="0.7"/>';

        // Subtle hexagon pattern (tech/innovation symbol)
        $svg .= '<polygon points="950,50 980,35 1010,50 1010,80 980,95 950,80" fill="rgba(59, 130, 246, 0.08)" opacity="0.4"/>';
        $svg .= '<polygon points="1080,400 1100,390 1120,400 1120,420 1100,430 1080,420" fill="rgba(6, 182, 212, 0.06)" opacity="0.5"/>';
      } else {
        // Default decorative elements for other themes
      ...
      // Professional achievement indicator (subtle) - only for Gabriel theme
      if ($colors === $this->color_schemes['gabriel']) {
        $svg .= '<rect x="320" y="340" width="8" height="8" rx="4" fill="' . $colors['accent_secondary'] . '" opacity="0.6"/>';
        $svg .= '<text x="338" y="349" font-family="system-ui, -apple-system, sans-serif" font-size="12" font-weight="500" fill="' . $colors['text_secondary'] . '" opacity="0.6">';
        $svg .= 'Product Manager • PhD Student • Developer';
        $svg .= '</text>';
      }
